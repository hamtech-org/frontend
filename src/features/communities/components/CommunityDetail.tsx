...
export function CommunityDetail({ groupId }: { groupId: string }) {
  ...
  const generalNavItems = [
    ...
    { id: 'members', label: 'Mọi người', icon: Users },
  ];

  const adminNavItems = [
    { id: 'analytics', label: 'Thống kê', icon: BarChart2 },
    { id: 'moderation', label: 'Duyệt bài viết', icon: ShieldCheck, count: pendingPostsCount },
    {
      id: 'requests',
      label: 'Yêu cầu duyệt',
      icon: UserPlus,
      count: (requests?.data ?? []).length,
      show: community?.joinPolicy === 'approval',
    },
    { id: 'reports', label: 'Báo cáo vi phạm', icon: Flag, count: pendingReportsCount },
    {
      id: 'automod',
      label: 'Bộ lọc từ khóa',
      icon: ShieldAlert,
      show: community?.viewerRole === 'owner' || community?.viewerRole === 'admin',
    },
    { id: 'moderation-logs', label: 'Nhật ký kiểm duyệt', icon: History },
  ];

  const mainClassName =
    'mx-auto flex max-w-[1440px] flex-col lg:flex-row gap-6 px-4 py-6 animate-in fade-in duration-300';

  return (
    <main className={mainClassName}>
      <aside className="hidden lg:flex flex-col gap-6 sticky top-0 h-[calc(100vh-120px)] w-[280px] shrink-0 bg-card/65 backdrop-blur-xl border border-border/40 rounded-3xl p-4 shadow-sm overflow-y-auto scrollbar-hide">
        <div className="flex items-center gap-3 px-1 pb-2 border-b border-border/20">
          <CommunityAvatar community={community} className="size-10 rounded-xl" />
          <div className="min-w-0">
            <h2 className="font-extrabold text-foreground text-sm truncate">{community.name}</h2>
            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider mt-0.5">
              {community.viewerRole && ROLE_LABEL[community.viewerRole]
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider px-2.5 mb-1.5 block">
              DÀNH CHO THÀNH VIÊN
            </span>
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.count !== undefined && item.count > 0 && (
                        <span className="flex size-4.5 items-center justify-center rounded-full bg-destructive text-[9px] font-extrabold text-destructive-foreground animate-pulse">
                          {item.count}
                        </span>
                      )}
                    </button>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4 text-center sm:text-left">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5 font-medium">
                  Bạn đã được mời tham gia cộng đồng này. Hãy tham gia ngay để bắt đầu chia sẻ ý
                  tưởng và thảo luận cùng mọi người.
                </p>
              </div>
            </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="lg:hidden w-full flex justify-start items-center border-b border-border/60 bg-transparent p-0 rounded-none h-auto gap-6 sm:gap-8 overflow-x-auto scrollbar-hide">
            <button
              type="button"
        ...
        >
          <AlertDialogContent className="rounded-2xl max-w-md border border-destructive/20 shadow-lg">
            ...
            <AlertDialogFooter className="sm:flex-row gap-2">
              ...
              <AlertDialogAction
                disabled={
                  confirmGroupNameInput.trim() !== community?.name?.trim() ||
        ...
        <AlertDialog open={unlinkConfirmOpen} onOpenChange={setUnlinkConfirmOpen}>
          <AlertDialogContent className="rounded-2xl max-w-md border border-destructive/20 shadow-lg">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-center font-medium leading-relaxed">
                Bạn có chắc chắn muốn giải tán phòng chat của cộng đồng này? Toàn bộ tin nhắn và
                danh sách thành viên sẽ bị xóa vĩnh viễn khỏi hệ thống.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="sm:flex-row gap-2">
