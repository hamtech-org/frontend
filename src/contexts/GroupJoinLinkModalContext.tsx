...
export type GroupJoinLinkModalData = {
  ...
  /** Deep link `/join/:suffix`: sau khi đóng modal (backdrop / nút back / Esc) sẽ `navigate` tới path này. */
  replacePathOnClose?: string;
};

type GroupJoinLinkModalContextValue = {
...
const GroupJoinLinkModalContext = createContext<GroupJoinLinkModalContextValue | null>(null);
...
export function GroupJoinLinkModalProvider({ children }: { children: ReactNode }) {
  ...
  const openGroupJoinLinkModal = useCallback((next: GroupJoinLinkModalData) => {
    setData(next);
  }, []);
  ...
  return (
    <GroupJoinLinkModalContext.Provider value={value}>
      ...
      <GroupJoinLinkModal
        open={Boolean(data)}
        data={data}
        onClose={closeGroupJoinLinkModal}
        onOpenSharePicker={openShareGroupJoinLinkPicker}
      />
...
export function useGroupJoinLinkModal(): GroupJoinLinkModalContextValue {
  const ctx = useContext(GroupJoinLinkModalContext);
  if (!ctx) {
    throw new Error('useGroupJoinLinkModal must be used within GroupJoinLinkModalProvider');
  }
  return ctx;
}

export function useGroupJoinLinkModalOptional(): GroupJoinLinkModalContextValue | null {
